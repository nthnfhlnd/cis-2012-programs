...
var passwordCheckOld = function () //sets up a function that checks the password
{
    var stringPassword, arrayBadPasswords, stringOutput;
    //defines the variables the function uses
    
    stringPassword = $("password").value;
    //takes input from the html file as the value for a variable 
    ...
    //produces the array made up of the commonly used bad passwords
    
    stringOutput = "";
    //deletes any output previously put out by the function in the html
 
 //the following starts a loop to make sure a password is not common and is enough characters
  if (stringPassword.length < 8) 
  {
    $("output").value = "That password is not 8 characters long. Please choose a password that is at least 8 characters long!";
    //first part of loop, returns error message when password isn't enough characters long
  } else {
   for (var a = 0; a < arrayBadPasswords.length; a++) //sets up loop to check the password against common passwords
   {
    if (stringPassword === arrayBadPasswords[a])
    {
        $("output").value = "That password has enough characters but is a bad password. Please choose a better password!";
        //outputs error message when password matches a common password
        break; //breaks out of the loop
    } else {
        $("output").value = "That is a good password!";
        //when a password meets the length requirement and doesn't match a common password, a success message is shown
    }
   }
  }
};
